...
- **Cible de diffusion** — Périmètre de destinataires d'un inject. Croisement de deux dimensions multi-valuées : périmètre d'équipes (instance entière ou liste) × filtre de rôles (tous ou liste). Toujours exprimée par un nom d'équipe ou de rôle, jamais par un nom propre.
